...
async function sendMaxMessage(userId, text, auctionId) {
    ...
    try {
        const response = await fetch(`https://platform-api.max.ru/messages?user_id=${userId}`, {
            method: 'POST',
            headers: {
                'Authorization': `Bearer ${config_1.config.maxBotToken}`,
...
app.post('/auth/max', async (req, res) => {
    const { max_user_id, first_name, last_name, username, role } = req.body;
    if (!max_user_id || !role) {
        return res.status(400).json({ error: 'max_user_id and role are required' });
    }
    const isAdminRequest = role === 'admin';
    if (isAdminRequest && !config_1.config.adminMaxUserIds.includes(Number(max_user_id))) {
        return res.status(403).json({ error: 'admin_access_denied' });
    }
    const client = await db_1.pool.connect();
    try {
        let userResult = await client.query('select * from users where max_user_id = $1', [max_user_id]);
        if (userResult.rowCount === 0) {
            const insertRole = isAdminRequest ? 'admin' : role;
            userResult = await client.query(`insert into users (max_user_id, role, first_name, last_name, username)
         values ($1, $2, $3, $4, $5)
         returning *`, [max_user_id, insertRole, first_name || null, last_name || null, username || null]);
        }
        else {
            const updates = isAdminRequest
                ? 'first_name = $2, last_name = $3, username = $4, role = \'admin\''
                : 'first_name = $2, last_name = $3, username = $4';
            userResult = await client.query(`update users set ${updates} where max_user_id = $1 returning *`, [max_user_id, first_name || null, last_name || null, username || null]);
        }
...
app.get('/users/profile', async (req, res) => {
    ...
    const client = await db_1.pool.connect();
    try {
        const result = await client.query(`select id, max_user_id, role, first_name, last_name, username,
...
app.post('/auctions', async (req, res) => {
    const { user_id, title, description, cargo_params, date_time, auction_ends_at } = req.body;
    ...
    try {
        ...
        const result = await client.query(`insert into auctions (customer_id, title, description, cargo_params, date_time, auction_ends_at, status)
       values ($1, $2, $3, $4, $5, $6, 'active')
       returning *`, [user_id, title, description || null, cargo_params || null, date_time, auction_ends_at]);
        const auction = result.rows[0];
        // Получаем всех грузчиков для рассылки
        const loadersResult = await client.query('select max_user_id, first_name, last_name from users where role = $1 and is_blocked = false', ['loader']);
...
app.get('/auctions/active', async (req, res) => {
    const client = await db_1.pool.connect();
    try {
        const result = await client.query(`select a.*, u.first_name, u.last_name, u.username,
...
app.post('/bids', async (req, res) => {
    const { auction_id, loader_id, amount } = req.body;
    ...
    try {
        ...
        const loaderResult = await client.query('select * from users where id = $1 and role = $2 and is_blocked = false', [loader_id, 'loader']);
        ...
        const auctionResult = await client.query('select * from auctions where id = $1 and status = $2 and auction_ends_at > now()', [auction_id, 'active']);
        if (auctionResult.rowCount === 0) {
            return res.status(404).json({ error: 'auction not found or not active' });
        }
        // Создаем или обновляем ставку
        const result = await client.query(`insert into bids (auction_id, loader_id, amount)
       values ($1, $2, $3)
       on conflict (auction_id, loader_id) 
       do update set amount = $3, created_at = now()
       returning *`, [auction_id, loader_id, amount]);
...
app.get('/admin/auth/status', (_req, res) => {
    res.json({ configured: !!(config_1.config.adminLogin && config_1.config.adminPassword) });
});
app.post('/admin/auth', (req, res) => {
    const login = typeof req.body?.login === 'string' ? req.body.login.trim() : '';
    const password = typeof req.body?.password === 'string' ? req.body.password.trim() : '';
    if (!config_1.config.adminLogin || !config_1.config.adminPassword) {
        return res.status(503).json({ error: 'admin_login_not_configured' });
    }
    if (login !== config_1.config.adminLogin || password !== config_1.config.adminPassword) {
...
async function requireAdmin(req) {
    ...
    if (bearer && isAdminTokenValid(bearer))
        return { ok: true };
    const adminUserId = Number(req.query.admin_user_id);
    if (!adminUserId)
        return { ok: false, status: 401, body: { error: 'admin auth required (Bearer token or admin_user_id)' } };
    const client = await db_1.pool.connect();
    try {
...
app.get('/admin/users', async (req, res) => {
    ...
    const client = await db_1.pool.connect();
    try {
        const users = await client.query(`select u.id, u.max_user_id, u.role, u.first_name, u.last_name, u.username,
...
app.get('/admin/users/:id', async (req, res) => {
    ...
    const client = await db_1.pool.connect();
    try {
        const u = await client.query('select * from users where id = $1', [targetId]);
        if (u.rowCount === 0)
            return res.status(404).json({ error: 'user not found' });
        const user = u.rows[0];
...
app.post('/admin/users/:id/block', async (req, res) => {
    ...
    if (!check.ok)
        return res.status(check.status).json(check.body);
    const targetId = Number(req.params.id);
    const { blocked, reason, until } = req.body;
    if (typeof blocked !== 'boolean')
        return res.status(400).json({ error: 'blocked (boolean) required' });
    const client = await db_1.pool.connect();
    try {
        if (blocked) {
            await client.query('update users set is_blocked = true, block_reason = $2, block_until = $3 where id = $1', [targetId, reason || null, until ? new Date(until).toISOString() : null]);
        }
        else {
            await client.query('update users set is_blocked = false, block_reason = null, block_until = null where id = $1', [targetId]);
        }
